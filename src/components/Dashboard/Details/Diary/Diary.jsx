    ...
    <List>
      {progress?.map(item => (
        <Event>
          <Line></Line>
          <div>
            <Data>{formatDate(item.finishReading)}</Data>
            <Percentage>
              {calculatePercentage(
            ...
            <Time>{readingTime(item.finishReading, item.startReading)}</Time>
          </div>
          <Box>
            <Container>
              <Pages>{readPages(item.startPage, item.finishPage)}</Pages>
              <GrafficSvg>
                <use xlinkHref="./svg/svgSprite.svg#icon-grafic"></use>
              </GrafficSvg>
              <Speed>{item.speed} pages per hour</Speed>
            </Container>
